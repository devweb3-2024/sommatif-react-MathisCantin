...
function App() {
  ...
  return (
    <>
        {gagnerMessage && (
          <div> Bravo, vous avez réussi !</div>
        )}
        {perduMessage && (
        ...
        <div className='rangee'>
          {RangeeImage1.map((image, index) => (
              <Carte dessus={dessus} clef={index+"row1"} photo={image as string} reduireEssai={reduireEssai}/>
          ))}
        </div>

        <div className='rangee'>
          {RangeeImage2.map((image, index) => (
            <Carte dessus={dessus} clef={index+"row2"} photo={image as string} reduireEssai={reduireEssai}/>
          ))}
        </div>

        <div className='rangee'>
          {RangeeImage3.map((image, index) => (
              <Carte dessus={dessus} clef={index+"row3"} photo={image as string} reduireEssai={reduireEssai}/>
          ))}
        </div>

        <div className='rangee'>
          {RangeeImage4.map((image, index) => (
              <Carte dessus={dessus} clef={index+"row4"} photo={image as string} reduireEssai={reduireEssai}/>
          ))}
        </div>
    </>
  );
